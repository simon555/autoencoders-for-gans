...
# =============================================================================
# ADDING CODE THAT WAS NOT HERE DURING THE TRAINING
# =============================================================================
    def decode(self,code):
        self.code_size=code.size()[1]//self.depth
        
        
        encoder_outs = []
        for i,_ in enumerate(self.down_convs):
            begin=(i)*self.code_size
            end=(i+1)*self.code_size
            encoder_outs.append(code[:,begin:end,:,:].clone())
            print('part ',i,' ',encoder_outs[i].size())

        x=encoder_outs[-1]
        print(x.size())
        for i, module in enumerate(self.up_convs):
            before_pool = encoder_outs[-(i+2)]     
            print('before pool ', before_pool.size())
            print('x ',x.size())
    ...
    def forward(self, x):
        ...
        for i, module in enumerate(self.up_convs):
            print('in forward, before :',before_pool.size())
            print('in forward, x :',x.size())
